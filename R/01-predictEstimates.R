...
# @return Data frame with predictions and uncertainties
summarize_bayes_predictions <- function(Predictions,
                                        model,
                                        XPred,
                                        estType = "Mean",
                                        estQuantile = 0.9,
  ...
  if (is_plot_time_course) {
    ...
    Est <- switch(
      ...
      "QuantileTOTAL" = rowMeans(
        Predictions + qnorm(estQuantile) * sqrt(PredictionsSigma^2 + apply(Predictions, 1, sd)^2)
      ),
  ...
  if (is_plot_time_course) {
    IntLower <- pmax(minVal, pmin(
      maxVal,
      rowMeans(Predictions) - sdValue * apply(Predictions, 1, sd)
    ))
    IntUpper <- pmax(minVal, pmin(
      maxVal,
      rowMeans(Predictions) + sdValue * apply(Predictions, 1, sd)
    ))
  } else {
    ...
    qs2 <- apply(
      Predictions + sqrt(PredictionsSigma^2 + apply(Predictions, 1, sd)^2),
      1,
      quantile,
      c(0.025, 0.975),
      names = FALSE
...
# @param maxVal Maximum value for predictions (for truncation)
# @return Data frame with predictions and uncertainties
summarize_gamm_predictions <- function(Est,
                                       model,
                                       XPred,
                                       estType = "Mean",
                                       estQuantile = 0.9,
  ...
  if (is_plot_time_course) {
    data.frame(
      ...
      Est = Est$fit,
      Sd = Est$se.fit,
      SdTotal = sqrt(Est$se.fit^2 +  mean(residuals(
        model$model$gam
      )^2)),
...
#  (if NULL, credible intervals are computed)
# @return Data frame with predictions and uncertainties
summarize_gam_predictions <- function(Predictions,
                                      model,
                                      XPred,
                                      estType = "Mean",
                                      estQuantile = 0.9,
